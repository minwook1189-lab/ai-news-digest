...
- 주요 AI 뉴스 사이트 RSS 피드에서 최신 기사 자동 수집 (최근 48시간, 소스당 최대 3건 / 커뮤니티 소스는 AI 키워드 필터 후 최대 3건)
- Groq API(Qwen3 32B)를 활용한 한국어 요약 및 카테고리 분류
- 커뮤니티 출처 기사에 ⚠️ 신뢰도 경고 자동 표시
- Outlook COM 자동화를 통한 HTML 이메일 발송 (다수 수신자 지원)
- Windows 작업 스케줄러로 평일(월~금) 오전 9시 자동 실행 (Outlook 실행 중일 때만 발송)
...
- 평일(월~금) 오전 9시에 **PC가 켜져 있고 Outlook이 실행 중인 상태**여야 합니다. Outlook이 꺼져 있으면 발송을 건너뜁니다.
- 회사 SSL 프록시(기업 방화벽) 환경에서는 Groq API 연결 시 SSL 검증을 우회합니다.
- `.env` 파일은 절대 GitHub에 업로드하지 마세요. (`.gitignore`에 포함되어 있습니다)
